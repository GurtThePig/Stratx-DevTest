...
local MainLink = LoadLocal and "" or "https://raw.githubusercontent.com/GurtThePig/Strategies-X/main/TDS/"
...
StratXLibrary.UtilitiesConfig = {
	...
	Webhook = {
		Enabled = true,
		Link = if (#tostring(getgenv().WebhookLink) ~= 0 and getgenv().WebhookLink ~= nil) then tostring(getgenv().WebhookLink) else "",
		HideUser = false,
		UseNewFormat = false,
		PlayerInfo = true,
		GameInfo = true,
...
if isfile("StrategiesX/TDS/UserConfig/UtilitiesConfig.txt") then
	...
	if #tostring(getgenv().WebhookLink) ~= 0 and getgenv().WebhookLink ~= nil and type(getgenv().WebhookLink) == "string" then
...
function SaveUtilitiesConfig()
	...
	StratXLibrary.UtilitiesConfig = {
		...
		Webhook = {
			...
			Link = (#WebSetting.flags.Link ~= 0 and WebSetting.flags.Link) or (#tostring(getgenv().WebhookLink) ~= 0 and getgenv().WebhookLink ~= nil and tostring(getgenv().WebhookLink)) or "",
